...
function Login(props) {
  ...
  return <KeyboardAvoidingView>
    ...
    <View style={styles.bodyContainer}>
      ...
      <Text style={styles.txt2}>Đăng nhập hoặc tạo tài khoản</Text>
      ...
      <TextInput style={styles.inputText}
       onChangeText={(text)=>{
        setErrorPassword(isValidPassword(text)==true ? '':"Password must be at least 3 charaters")
        setPassword(text)
      }}
      ...
      <TouchableOpacity onPress={() => { alert() }}>
        ...
        <Text style={styles.btnemail}>Đăng ký</Text>
      </TouchableOpacity>
      <View style={styles.form}>
        <Text  style={{color:"#adaaaa"}} >Hoặc tiếp tục bằng</Text>
        <View style={styles.formdn}>
         <Icon name="facebook" size={30} color={"#1867de"} />
         <View style={{width:20}}/>
         <Icon name="envelope" size={30} color={"#1867de"}/>
         <View style={{width:20}}/>
         <Icon name="comment" size={30} color={"#1867de"}/>
        </View>
        <Text  style={{color:"#adaaaa"}} >Bằng việc tiếp tục bạn đã chấp nhận điều khoản sử dụng</Text>
      </View>
    </View>
